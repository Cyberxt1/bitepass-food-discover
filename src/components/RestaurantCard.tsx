...
export function RestaurantCard({ r, distanceLabel }: { r: Restaurant; distanceLabel?: string }) {
  return (
    ...
    >
      ...
      <div className="relative border-b border-border bg-muted/35 p-2.5">
        {!r.image && <p className="line-clamp-2 min-h-10 text-base font-black leading-tight">{r.name}</p>}
        <p className="mt-1 line-clamp-1 text-xs leading-5 text-muted-foreground">{r.description}</p>
      ...
      <div className="p-2.5">
        ...
        {r.address && (
          <p className="mt-1.5 flex items-start gap-1 text-[11px] font-semibold leading-4 text-foreground">
